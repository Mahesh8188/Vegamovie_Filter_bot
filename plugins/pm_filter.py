...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"📁 {get_size(file.file_size)}≽ @akimax06 {formate_file_name(file.file_name)}", url=f'https://telegram.dog/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}'),]
                for file in files
              ]
    btn.insert(0,[
...
async def season_search(client: Client, query: CallbackQuery):
  ...
  #  del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    ...
    if settings["link"]:
        ...
        btn = [[
                InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ @akimax06 {formate_file_name(file.file_name)}", callback_data=f'cfiles#{reqnxt}#{file.file_id}'),]
                   for file in files
              ]
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ @akimax06 {formate_file_name(file.file_name)}", url=f'https://telegram.dog/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'),]
               for file in files
              ]
    if offset != "":
