...
from info import ADMINS, AUTH_REQ_CHANNELS, AUTH_CHANNELS
...
def is_auth_channel(_, __, update):
    return update.chat.id in AUTH_CHANNELS

@Client.on_chat_join_request(create(is_auth_channel))
async def join_reqs(client, message: ChatJoinRequest):
    await db.add_join_req(message.from_user.id, message.chat.id)
